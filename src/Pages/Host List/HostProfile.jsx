...
import HostProfileTop from './HostProfileTop';
import HostCarsdetails from './HostCarsdetails';
import HostOffers from './HostOffers';
import PaymentPayout from './PaymentPayout';

const HostProfile = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const [ishostdocument, setIsHostDocument] = useState(false)
    const [ishostbooking, setIsHostBookings] = useState(false)
    const [ishostpayment, setIsHostPayment] = useState(false)
    const [ishostcars, setIsHostCars] = useState(false)
    const [ishostOffers, setIsHostoffers] = useState(false)
        ...
        <>
            ...
            <div className='dashboard'>
                <div className='userprofile'>
                    <div className='userprofile2'>
                        <button onClick={() => navigate('/hostlist')}>Back</button>
                    </div>
                    <div>
                        ...
                        <HostProfileTop data={userdata} />
                    </div>
                    <div>
                        <div className="hostprofile6">
                            <div className="hostprofile7">
                                <p className={ishostdocument ? "hostprofile8" : "hostprofile9"} onClick={() => {
                                    setIsHostDocument(true);
                                    setIsHostCars(false)
                                    setIsHostPayment(false)
                                    setIsHostBookings(false)
                                    setIsHostoffers(false)
                                }}>Document</p>
                                <p className={ishostcars ? "hostprofile8" : "hostprofile9"} onClick={() => {
                                    setIsHostDocument(false);
                                    setIsHostCars(true)
                                    setIsHostPayment(false)
                                    setIsHostBookings(false)
                                    setIsHostoffers(false)
                                }}>Listed Cars</p>
                                <p className={ishostpayment ? "hostprofile8" : "hostprofile9"} onClick={() => {
                                    setIsHostDocument(false);
                                    setIsHostCars(false)
                                    setIsHostPayment(true)
                                    setIsHostBookings(false)
                                    setIsHostoffers(false)
                                }}>Payment and Payout</p>
                                <p className={ishostbooking ? "hostprofile8" : "hostprofile9"} onClick={() => {
                                    setIsHostDocument(false);
                                    setIsHostCars(false)
                                    setIsHostPayment(false)
                                    setIsHostBookings(true)
                                    setIsHostoffers(false)
                                }}>Rides</p>
                                <p className={ishostOffers ? "hostprofile8" : "hostprofile9"} onClick={() => {
                                    setIsHostDocument(false);
                                    setIsHostCars(false)
                                    setIsHostPayment(false)
                                    setIsHostBookings(false)
                                    setIsHostoffers(true)
                                }}>Offers</p>
                            </div>
                        </div>
